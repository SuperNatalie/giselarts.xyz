...
$(document).ready(function () {
  ...
  GetSheetDone.labeledCols("1zLUp-zc3MEn845mynUHoCBDPKeGPF7IGL_cC2pZ8gOI", 1)
    .then((sheet) => {
      console.log(sheet);
      for (const product of sheet.data) {
        let galleryCaption = product.bildtitel
          .replace("\n", "<br />")
          .replace("\n", "<br />")
          .replace("\n", "<br />");
        let modalCaption = product.bildtitel
          .replace("\n", " | ")
          .replace("\n", " | ")
          .replace("\n", " | ");

        sheetPic.innerHTML += `
        <figure class="effect-chico tm-gallery-item ${product.category}" onclick="showModal('${product.bildlink}', '${modalCaption}')">
    ...
    .then(() => {
      ...
      function setPagination() {
        ...
        var CreatePagers = (function () {
          var $isotopePager =
            $("." + pagerClass).length == 0
              ? $('<div class="' + pagerClass + ' tm-paging"></div>')
              : $("." + pagerClass);
          $isotopePager.html("");
          if (currentNumberPages > 1) {
            for (var i = 0; i < currentNumberPages; i++) {
              ...
              if (currentPage == i + 1) {
                $pager = $(
                  '<a href="javascript:void(0);" class="pager active tm-paging-link" ' +
                    pageAttribute +
                    '="' +
                    (i + 1) +
                    '"></a>'
                ...
                $pager = $(
                  '<a href="javascript:void(0);" class="pager tm-paging-link" ' +
                    pageAttribute +
                    '="' +
                    (i + 1) +
                    '"></a>'
      ...
      document.getElementById("tm-video").playbackRate = 0.7;
      setVideoSize();
      ...
      function adjustIntroImg() {
        ...
        if (window.innerWidth > 650) {
          img += "gise-bild-about-small.jpg";
        } else {
          img += "gise-bild-about.jpg";
        }
